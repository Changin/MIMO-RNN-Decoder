...
mmse_list = Comparison.iloc[2, 1:]    # MMSE 데이터
Comparison_snr = Comparison.iloc[0, 1:]     # 비교성능 데이터 해당 SNR

# ber 그래프
plt.semilogy(dB_snr, avg_ber_list, marker='o', color='orange', label='LSTM')
plt.semilogy(Comparison_snr, perfect_channel_list, marker='x', color='blue', label='perfect_channel')    # 비교성능.xlsx의 BER_perfect_channel 그래프 추가
plt.semilogy(Comparison_snr, mmse_list, marker='s', color='green', label='MMSE')
plt.xticks(dB_snr)
